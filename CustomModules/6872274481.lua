...
local store = {
	...
	whitelist = {
		chatStrings1 = {skidinhaler = "vape"},
		chatStrings2 = {vape = "skidinhaler"},
		clientUsers = {},
		oldChatFunctions = {}
	},
	zephyrOrb = 0
...
local function vapeGithubRequest(scripturl)
	if not isfile("vape/"..scripturl) then
		local suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/spookzv4/SpookzV4/"..readfile("vape/commithash.txt").."/"..scripturl, true) end)
		assert(suc, res)
...
run(function()
	...
	local oldclickhold2
	local roact
	local FastConsume = wingui.blatant({
		Name = "FastConsume",
		Function = function(callback)
			if callback then
							...
							if sword then
								...
								for i, plr in pairs(plrs) do
									...
									killaurarealremote:FireServer({
										...
										validate = {
											raycast = {
												cameraPosition = attackValue(root.Position + plr.Character.Humanoid.MoveDirection + Vector3.new(0, 0.003, 0)),
...
run(function()
	...
	local BowAimbot = wingui.blatant({
                ...
                task.spawn(function()
                    repeat
                        ...
                        if DragonSound.Enabled then
                            game:GetService("ReplicatedStorage").rbxts_include.node_modules["@rbxts"].net.out._NetManaged.DragonBreath:FireServer(" ") -- i think i can make this better, lets see
                        end
...
--[[run(function()
	...
	JellyfishExploit = wingui.exploit({
